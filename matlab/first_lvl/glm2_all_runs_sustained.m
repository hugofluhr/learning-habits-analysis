...
output_dir = fullfile(analysis_dir, 'outputs', ['glm2_all_runs_sustained_' current_date]);
...
for s = 1:length(subjects)
    ...
    for r = 1:3 % all runs (learning1, learning2, test)
        ...
        % handle duration - different for the first stimulus
        duration = block_data.t_action - block_data.t_first_stim;
        duration(duration < 0) = 1 + block_data.t_second_stim(duration<0) - block_data.t_first_stim(duration<0); % for non-response trials
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(1).duration = duration;
