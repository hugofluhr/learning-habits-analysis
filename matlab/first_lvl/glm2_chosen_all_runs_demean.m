...
output_dir = fullfile(analysis_dir, 'outputs', ['glm2_chosen_all_runs_scrubbed_demeaned_' current_date]);
...
for s = 1:length(subjects)
    ...
    % Compute subject-level pmod means across all runs, response trials only (for demeaning)
    sub_data = bbt(strcmp(bbt.sub_id, sub_id), :);
    sub_resp = sub_data(~isnan(sub_data.action), :);
    mean_chosen_rl = mean(sub_resp.chosen_value_rl, 'omitnan');
    mean_chosen_ck = mean(sub_resp.chosen_value_ck, 'omitnan');
    ...
    for r = 1:3 % all runs (learning1, learning2, test)
        ...
        % Demean pmods using subject-level mean across all runs (response trials only)
        block_resp.chosen_value_rl_demean = block_resp.chosen_value_rl - mean_chosen_rl;
        block_resp.chosen_value_ck_demean = block_resp.chosen_value_ck - mean_chosen_ck;
        ...
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(2).pmod(1).param = block_resp.chosen_value_rl_demean;
        ...
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(2).pmod(2).param = block_resp.chosen_value_ck_demean;
