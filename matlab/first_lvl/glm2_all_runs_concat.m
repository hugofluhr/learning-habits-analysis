...
if ~exist('output_dir', 'var') || isempty(output_dir)
    output_dir = fullfile(analysis_dir, 'outputs', ['glm2_all_runs_concat_scrubbed_' current_date]);
end
...
for s = 1:length(subjects)
    ...
    for r = 1:numel(sess)
        ...
        nscan(r) = size(vols, 1);
        scans = [scans; cellstr(vols)];
        conf{r} = load(sess(r).multi_reg{1});
    end
    ...
    for c = 1:numel(merged.cond)
        ...
        for r = 1:numel(sess)
            ...
            if isscalar(d); d = repmat(d, numel(sess(r).cond(ic).onset), 1); end % scalar duration (e.g. response = 0)
            duration = [duration; d];
            for p = 1:numel(merged.cond(c).pmod)
                if r == 1
                    merged.cond(c).pmod(p).param = [];
                end
                merged.cond(c).pmod(p).param = [merged.cond(c).pmod(p).param; sess(r).cond(ic).pmod(p).param];
            end
        end
        merged.cond(c).onset = onset;
        merged.cond(c).duration = duration;
    end

    % Confounds: block-diagonal, each run's confounds in their own columns
    multi_reg_file = fullfile(sub_output_dir, 'concat_confounds.txt');
    R = blkdiag(conf{:});
    save(multi_reg_file, 'R', '-ascii');
    merged.multi_reg = {multi_reg_file};
    matlabbatch{1}.spm.stats.fmri_spec.sess = merged;
    ...
    save(fullfile(sub_output_dir, 'batch_spec.mat'), 'matlabbatch');
    spm_jobman('run', matlabbatch);

    % One constant per run, per-run high-pass filter and AR(1); must happen between specification and estimation
    spm_fmri_concatenate(fullfile(sub_output_dir, 'SPM.mat'), nscan);

    %% Model estimation
    clear matlabbatch
    matlabbatch{1}.spm.stats.fmri_est.spmmat = {fullfile(sub_output_dir, 'SPM.mat')};
    matlabbatch{1}.spm.stats.fmri_est.write_residuals = 0;
    matlabbatch{1}.spm.stats.fmri_est.method.Classical = 1;

    save(fullfile(sub_output_dir, 'batch_est.mat'), 'matlabbatch');
    spm_jobman('run', matlabbatch);
    ...
    for ec = 1:size(extra_contrasts, 1)
        ...
        matlabbatch_con{1}.spm.stats.con.consess{end}.tcon.sessrep   = 'none'; % single session now
    end
