...
% Optional explicit subject-id override (e.g. {'sub-01'}) for a smoke test;
% leave empty to process every subject in bbt_path.
if ~exist('subjects_override', 'var')
    subjects_override = {};
end
addpath(spmpath);

% Source columns for the second_stim pmods. Switch between the
% across-runs normalisation (*_zscore) and the within-session
% normalisation (*_run_zscore) by editing this block only.
pmod_cols = struct( ...
    'Qval1', 'first_stim_value_rl_zscore', ...
    'Hval1', 'first_stim_value_ck_zscore', ...
    'Qval2', 'second_stim_value_rl_zscore', ...
    'Hval2', 'second_stim_value_ck_zscore');
% To use within-session normalisation instead, use e.g.:
%   'Qval1', 'first_stim_value_rl_run_zscore', ...

current_date = char(datetime('now', 'Format', 'yyyy-MM-dd-hh-mm'));
output_dir = fullfile(analysis_dir, 'outputs', ['glm2_all_runs_diff_timing_scrubbed_' current_date]);
...
% Condition order matches the design matrix column order for cond(1)-cond(4)
% below (verified against the fmri_spec.sess(r).cond assignments):
%   1 first_stim, 2 second_stim, 3 second_stimxQval1, 4 second_stimxHval1,
%   5 second_stimxQval2, 6 second_stimxHval2, 7 response, 8 purple_frame
connames = {
    'first_stim', 'second_stim', ...
    'second_stimxQval1', 'second_stimxHval1', ...
    'second_stimxQval2', 'second_stimxHval2', ...
    'response', 'purple_frame'
    };

% Sum/difference contrasts over the second_stim pmods. Weight positions
% follow the 6-column block [first_stim, second_stim, Qval1, Hval1,
% Qval2, Hval2], which is identical across all three sessions since it
% precedes the session-dependent points_feedback/nresp_screen columns.
extra_contrasts = {
    'Qval_sum',  [0 0 1 0 1 0];
    'Hval_sum',  [0 0 0 1 0 1];
    'Qval_diff', [0 0 1 0 -1 0];
    'Hval_diff', [0 0 0 1 0 -1]
    };
...
for s = 1:length(subjects)
    ...
    %% ===========================
    %% FULL MODEL (run01+run02+run03)  ->  outputs/<sub>/
    %% ===========================
    ...
    for r = 1:3 % all runs (learning1, learning2, test)
        ...
        % First stimulus - All trials - no modulators
        ...
        % Second stimulus - All trials - All modulators here
        ...
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(2).pmod(1).name = 'Qval1';
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(2).pmod(1).param = block_data.(pmod_cols.Qval1);
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(2).pmod(1).poly = 1;
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(2).pmod(2).name = 'Hval1';
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(2).pmod(2).param = block_data.(pmod_cols.Hval1);
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(2).pmod(2).poly = 1;
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(2).pmod(3).name = 'Qval2';
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(2).pmod(3).param = block_data.(pmod_cols.Qval2);
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(2).pmod(3).poly = 1;
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(2).pmod(4).name = 'Hval2';
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(2).pmod(4).param = block_data.(pmod_cols.Hval2);
    ...
    % Contrasts (combined, using sessrep = 'repl')
    ...
    % because of volume censoring (differing confound counts) and because
    % the test session has no points_feedback condition
    ...
    for cc = 1:length(connames)
        ...
        if isempty(idx)
            warning('No regressor found for %s in session 1', connames{cc});
            continue
        end
        w(idx) = 1;
