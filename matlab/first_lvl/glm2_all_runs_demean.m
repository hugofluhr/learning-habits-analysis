...
output_dir = fullfile(analysis_dir, 'outputs', ['glm2_all_runs_scrubbed_demeaned_' current_date]);
...
for s = 1:length(subjects)
    ...
    % Compute subject-level pmod means across all runs (for demeaning)
    sub_data = bbt(strcmp(bbt.sub_id, sub_id), :);
    mean_first_rl  = mean(sub_data.first_stim_value_rl,  'omitnan');
    mean_first_ck  = mean(sub_data.first_stim_value_ck,  'omitnan');
    mean_second_rl = mean(sub_data.second_stim_value_rl, 'omitnan');
    mean_second_ck = mean(sub_data.second_stim_value_ck, 'omitnan');
    ...
    for r = 1:3 % all runs (learning1, learning2, test)
        ...
        % Demean pmods using subject-level mean (across all runs)
        block_data.first_stim_value_rl_demean  = block_data.first_stim_value_rl  - mean_first_rl;
        block_data.first_stim_value_ck_demean  = block_data.first_stim_value_ck  - mean_first_ck;
        block_data.second_stim_value_rl_demean = block_data.second_stim_value_rl - mean_second_rl;
        block_data.second_stim_value_ck_demean = block_data.second_stim_value_ck - mean_second_ck;
        ...
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(1).pmod(1).param = block_data.first_stim_value_rl_demean;
        ...
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(1).pmod(2).param = block_data.first_stim_value_ck_demean;
        ...
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(2).pmod(1).param = block_data.second_stim_value_rl_demean;
        ...
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(2).pmod(2).param = block_data.second_stim_value_ck_demean;
