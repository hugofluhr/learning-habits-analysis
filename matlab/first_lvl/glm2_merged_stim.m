...
output_dir = fullfile(analysis_dir, 'outputs', ['glm2_merged_stim_scrubbed_' current_date]);
...
for s = 1:length(subjects)
    ...
    for r = 1:3 % all runs (learning1, learning2, test)
        ...
        % Single merged stimulus condition pooling first and second stimulus events.
        % Onsets and pmods are concatenated [first_stim(1:N); second_stim(1:N)], giving
        % a 2N-length vector. SPM builds the design matrix in time (not vector order),
        % so onset ordering is irrelevant — what matters is that index i is consistent
        % across onset, duration, and both pmod vectors, which the concatenation guarantees.
        dur_first  = block_data.t_second_stim - block_data.t_first_stim;
        dur_second = block_data.t_action - block_data.t_second_stim;
        dur_second(dur_second < 0) = 1; % non-response trials have no t_action; clamp to 1s
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(1).name     = 'stim';
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(1).onset    = [block_data.t_first_stim; block_data.t_second_stim];
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(1).duration = [dur_first; dur_second];
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(1).tmod     = 0;
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(1).pmod(1).name  = 'Qval';
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(1).pmod(1).param = [block_data.first_stim_value_rl_zscore; block_data.second_stim_value_rl_zscore];
        ...
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(1).pmod(2).param = [block_data.first_stim_value_ck_zscore; block_data.second_stim_value_ck_zscore];
        ...
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(2).name = 'response';
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(2).onset = block_resp.t_action;
        ...
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(3).name = 'purple_frame';
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(3).onset = block_resp.t_purple_frame;
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(3).duration = block_resp.t_points_feedback - block_resp.t_purple_frame;
        ...
        if r == 3
            ...
            k = 4; % next condition index
        else
            % Learning runs
            matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(4).name = 'points_feedback';
            matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(4).onset = block_resp.t_points_feedback;
            matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(4).duration = block_resp.t_iti_onset - block_resp.t_points_feedback;
