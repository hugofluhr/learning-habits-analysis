...
bbt_path = '/home/ubuntu/data/learning-habits/bbt_20260401.csv';
addpath(spmpath);

current_date = char(datetime('now', 'Format', 'yyyy-MM-dd-hh-mm'));
output_dir = fullfile(analysis_dir, 'outputs', ['glm3_chosen_choice_var_scrubbed_' current_date]);
...
connames = {
    'first_stim', 'second_stim', 'second_stimxChoiceval_chosen', ...
...
for s = 1:length(subjects)
    ...
    for r = 1:3 % all runs (learning1, learning2, test)
        ...
        % Second stimulus - Resp trials - chosen choice variable pmod
        ...
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(2).pmod(1).name = 'Choiceval_chosen';
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(2).pmod(1).param = block_resp.chosen_choice_val_zscore;
        ...
        % Second stimulus - NoResp trials (no pmod, no choice made)
