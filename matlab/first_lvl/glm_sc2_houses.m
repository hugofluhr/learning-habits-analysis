...
output_dir = fullfile(analysis_dir, 'outputs', ['glm_sc2_houses_' current_date]);
...
bbt.first_stim_house = double(strcmp(bbt.first_stim_cat, 'house'));
bbt.second_stim_house = double(strcmp(bbt.second_stim_cat, 'house'));
...
for s = 1:length(subjects)
    ...
    for r = 1:3 % all runs (learning1, learning2, test)
        ...
        % Seeing house - from first appearance of a house to ITI onset
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(k+1).name = 'house';
        % house present if either stimulus is a house
        is_house = (block_data.first_stim_house == 1) | (block_data.second_stim_house == 1);
        % Compute onset per trial, then subset
        house_onset_all = block_data.t_first_stim;
        % If house appears only at second stim, onset should be second stim
        second_only_house = (block_data.first_stim_house == 0) & (block_data.second_stim_house == 1);
        house_onset_all(second_only_house) = block_data.t_second_stim(second_only_house);
        house_onset = house_onset_all(is_house);
        % Duration from house onset to ITI onset
        house_duration = block_data.t_iti_onset(is_house) - house_onset;
        house_duration(house_duration < 0) = 0; % safety
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(k+1).onset = house_onset;
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond(k+1).duration = house_duration;
