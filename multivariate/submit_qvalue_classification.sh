...
# pattern — LinearSVC classification is the same performance class as the category
# decoder). Resources below are a starting guess pending a real smoke-test measurement;
# update this comment once you have one.
#
...
echo "Submitting 1 job for ${N} subjects (NPROC=${NPROC} concurrent), target='${TARGET_COL}' (low<=${LOW_MAX}, high>=${HIGH_MIN}), bbt='${BBT}':"
cat "$SUBJECTS_FILE"
echo
