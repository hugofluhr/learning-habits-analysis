#!/bin/bash
# Submit reward-value (Q-value) searchlight regression as a SLURM array job —
# one job per subject. Regression counterpart of submit_searchlight.sh: decodes a
# continuous target (default first_stim_value, the objective reward level) from the
# GLMsingle CUES betas, with the target sourced from the BBT (same table used for
# the SPM first-levels).
#
# Usage (from repo root):
#   bash multivariate/submit_qvalue_searchlight.sh            # all subjects in PARTICIPANTS_TSV
#   bash multivariate/submit_qvalue_searchlight.sh 01 05 12   # specific subjects
#   BBT=/path/to/bbt.csv bash multivariate/submit_qvalue_searchlight.sh
#   TARGET_COL=first_stim_value_rl bash multivariate/submit_qvalue_searchlight.sh   # model RL Q instead
...
sbatch <<EOF
#!/bin/bash -l
#SBATCH --job-name=qval_searchlight
#SBATCH --output=${LOG_DIR}/qvalue_searchlight_%A_%a.out
#SBATCH --error=${LOG_DIR}/qvalue_searchlight_%A_%a.err
#SBATCH --ntasks=1
...
/home/hfluhr/data/conda/envs/learning-habits/bin/python -u "${REPO}/multivariate/run_qvalue_searchlight.py" \\
    --subject "\$SUBJECT" \\
    --bids-dir "${BIDS_DIR}" \\
