#!/bin/bash
# Submit choice-frequency (±1) classification as a SINGLE SLURM job that runs all
# subjects internally via `xargs -P` (same pattern as submit_qvalue_classification.sh —
# LinearSVC classification on ROI masks, seconds per subject).
#
# Uses the same 9 ROI masks as the RSA pipeline (submit_rsa_roi.sh) for direct comparison.
#
# Usage (from repo root):
#   bash multivariate/submit_frequency_decoding.sh            # all subjects in PARTICIPANTS_TSV
#   bash multivariate/submit_frequency_decoding.sh 01 05 12   # specific subjects
#   NPROC=4 bash multivariate/submit_frequency_decoding.sh    # override concurrency
#   OVERWRITE=1 bash multivariate/submit_frequency_decoding.sh
#
# run_frequency_decoding.py skips subjects already done, so this is resumable.
...
OUTPUT_DIR="/home/hfluhr/shares-hare/ds-learning-habits/derivatives/frequency_decoding"
...
# ROI masks — same 8 as submit_rsa_roi.sh; whole-brain is added automatically
...
sbatch <<EOF
#!/bin/bash -l
#SBATCH --job-name=freq_decode
#SBATCH --output=${LOG_DIR}/frequency_decoding_%j.out
#SBATCH --error=${LOG_DIR}/frequency_decoding_%j.err
#SBATCH --ntasks=1
...
run_one() {
    local s="\$1"
    /home/hfluhr/data/conda/envs/learning-habits/bin/python -u "${REPO}/multivariate/run_frequency_decoding.py" \
        --subject "\$s" \
        --base-dir "${BASE_DIR}" \
        --bids-dir "${BIDS_DIR}" \
...
echo "All frequency_decoding subjects finished. Per-subject logs under: ${OUTPUT_DIR}/sub-*/"
EOF
