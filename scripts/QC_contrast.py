...
derivatives_dir = os.path.join(base_dir, 'outputs/first_level/QC_cloud-23.2.1')
...
def process_subject(sub_id, tr, hrf_model, high_pass, smoothing_fwhm, derivatives_dir, mask_samples):
    ...
    try:
        ...
        for run in subject.runs:
            ...
            model_fit = model.fit(fmri_img, design_matrices=X1, sample_masks=sample_mask)
            # Compute betamap and save it
            z_map = model.compute_contrast(
                contrast_def=f"{'response'}", output_type="z_score"
            )
