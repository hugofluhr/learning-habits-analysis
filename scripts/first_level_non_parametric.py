...
from utils.analysis import run_model_non_parametric
...
derivatives_dir = os.path.join(base_dir, 'nilearn/non_parametric')
...
def process_subject(sub_id, tr, hrf_model, high_pass, smoothing_fwhm, derivatives_dir, mask_samples):
    ...
    try:
        ...
        for run in subject.runs:
            ...
            if not mask_samples:
                sample_mask = None
            run_model_non_parametric(subject, run, confounds, sample_mask, tr, hrf_model, high_pass, smoothing_fwhm,
                                     derivatives_dir, model_label='test1', modulator='reward', demean_modulator=demean_modulators,
                                     plot_stat=False, plot_design=True)
            run_model_non_parametric(subject, run, confounds, sample_mask, tr, hrf_model, high_pass, smoothing_fwhm,
                                     derivatives_dir, model_label='test1', modulator='frequ', demean_modulator=demean_modulators,
                                     plot_stat=False, plot_design=True)
...
if __name__ == "__main__":
    ...
    #process_subject('sub-01', tr, hrf_model, high_pass, smoothing_fwhm, derivatives_dir, mask_samples)
